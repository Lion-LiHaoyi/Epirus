...
while player[0].HP > 0 and player[1].HP > 0:
    ...
    check = 0
    while not check:
        player[1].skill = random.randint(0, len(skills) + 5)
        if(player[1].skill >= len(skills)):
            player[1].skill = 0
        check = player[1].Ep >= skills[player[1].skill].cost
        if player[1].skill == 9 and player[1].HP > 1:
            check = 0
        ...
        # 一方防御类
            ...
            if(player[(i + 1) & 1].skill == 5):
                if(random.randint(0, 1) == 0):
                    ...
                    print('判定成功')
                    break
            player[(i + 1) & 1].HP += table[player[i].skill -6][player[(i + 1) & 1].skill][0]
            player[i].HP += table[player[i].skill -6][player[(i + 1) & 1].skill][1]
        # 狙击
