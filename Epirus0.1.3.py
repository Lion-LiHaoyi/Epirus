...
attact_number1, lightning, snipe, bagua, gou = 0, 0, 0, 0, 0
...
while player[0].HP > 0 and player[1].HP > 0:
    ...
    for i in range(0, 2):
        ...
        if player[i].skill == gou:
            if player[i].HP <= 1:
                if player[(i + 1) & 1].skill == bagua:
                    ...
                    player[(i + 1) & 1].HP -= 1
        # 同时攻击类
        elif(player[i].skill >= attact_number1 and player[(i + 1) & 1].skill >= attact_number1):
        ...
        # 一方防御类
        elif(player[i].skill >= attact_number1):
            if player[(i + 1) & 1].skill == bagua:
                if(random.randint(0, 1) == 0):
                    print('判定失败')
                    player[(i + 1) & 1].skill = 0
                    ...
                    break
            player[(i + 1) & 1].HP += table[player[i].skill -
                                            attact_number1][player[(i + 1) & 1].skill][0]
            player[i].HP += table[player[i].skill -
                                  attact_number1][player[(i + 1) & 1].skill][1]
        # 狙击
